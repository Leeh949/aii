-- [III] JOIN: 2개이상 이상의 테이블을 연결하여 데이터를 검색하는 방법
...
-- ★1. EQUI JOIN★ 공동필드값이 일치되는 조건만 JOIN
...
SELECT EMPNO, ENAME, JOB, MGR, HIREDATE, SAL, COMM, D.*
    ...
    SELECT E.ENAME, D.DEPTNO, LOC
        FROM EMP E, DEPT D 
        WHERE E.DEPTNO = D.DEPTNO AND E.DEPTNO =20;
    -- EX. 근무지가(LOC)가 CHICAGO인 사람의 이름, 급여, 부서번호를 출력
    ...
    -- EX COMMDL NULL이고, SAL이 1200이상인 사원의 이름, 급여, 부서번호, 부서명
    -- 부서명, 급여 큰 순정렬
...
SELECT * FROM EMP, SALGRADE
    ...
    FROM EMP E, DEPT D, SALGRADE WHERE SAL BETWEEN LOSAL AND HISAL
    ORDER BY DNAME,ANNUALSAL; 
	
    -- 이름, 업무, 급여, 등급, 부서코드, 부서명 출력. 급여가 1000~3000사이. 정렬조건 : 부서별, 부서같으면 업무별, 업무같으면 급여 큰순
    SELECT ENAME,JOB, SAL, GRADE, DNAME, E.DEPTNO
    FROM EMP E, DEPT D, SALGRADE WHERE E.DEPTNO=D.DEPTNO AND SAL BETWEEN LOSAL AND HISAL
...
SELECT EMPNO, ENAME, SAL, DNAME FROM EMP E, DEPT D
    WHERE E.DEPTNO=D.DEPTNO AND SAL>= 2000 ORDER BY SAL;
...
SELECT ENAME,JOB , SAL, GRADE ||'등급' FROM EMP E, DEPT D, SALGRADE
...
SELECT WORKER.EMPNO, WORKER.ENAME, WORKER.MGR, MANAGER.EMPNO, MANAGER.ENAME 
    ...
    SELECT W.EMPNO, W.ENAME, M.MGR, M.ENAME FROM EMP M, EMP W
    WHERE W.MGR = M.EMPNO;
    
    
    
    -- 'SMITH'의 상사는 FORD다 포맷으로 출력
...
-- 4 OUTER JOIN: SELF-JOIN시 조건에 만족하니 않는 행까지 나타나게
-- (1) SLEF JOIN에서의 OUTER JOIN
SELECT W.ENAME, W.MGR, M.EMPNO, M.ENAME
    FROM EMP W, EMP M
    WHERE W.MGR = M.EMPNO(+);
    -- 'SMITH의 상사는 FORD다'- 'KING의 상사는 없다'
...
SELECT W.ENAME, W.SAL , M.ENAME FROM EMP W, EMP M
WHERE W.MGR = M.EMPNO;
--4. 이름, 급여, 부서명, 직속상사명
SELECT W.ENAME, W.SAL , DNAME , M.ENAME FROM EMP W, DEPT, EMP M;
--5. 상사가 없는 직원과 상사가 있는 직원 모두에 대해 이름, 급여, 부서코드, 부서명, 근무지, 직속상사명을 출력하시오(단, 직속상사가 없을 경우 직속상사명에는 ‘없음’으로 대신 출력하시오)
SELECT W.ENAME, W.SAL, W.DEPTNO, D.DNAME, D.LOC, NVL(M.ENAME,'없음') AS 직속상사
    FROM EMP W, EMP M, DEPT D 
    WHERE W.MGR = M.EMPNO(+) AND W.DEPTNO = D.DEPTNO; 
--6. 이름, 급여, 등급, 부서명, 직속상사명. 급여가 2000이상인 사람
SELECT W.ENAME, W.SAL, S.GRADE||'등급', D.DNAME, M.ENAME 
...
SELECT W.ENAME, M.ENAME ,MM.ENAME FROM EMP M, EMP W, EMP MM
